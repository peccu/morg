...
async function applyAction(token: string, req: BatchOperationRequest) {
  ...
  switch (action) {
    case 'archive':
      return runBatched(threadIds.map((id) => () => modifyThread(token, id, [], ['INBOX'])))
    case 'trash':
      return runBatched(threadIds.map((id) => () => trashThread(token, id)))
    case 'markRead':
      return runBatched(threadIds.map((id) => () => modifyThread(token, id, [], ['UNREAD'])))
    case 'markUnread':
      return runBatched(threadIds.map((id) => () => modifyThread(token, id, ['UNREAD'], [])))
    case 'addLabel':
      if (!labelId) throw new Error('labelId required')
      return runBatched(threadIds.map((id) => () => modifyThread(token, id, [labelId], [])))
    case 'removeLabel':
      if (!labelId) throw new Error('labelId required')
      return runBatched(threadIds.map((id) => () => modifyThread(token, id, [], [labelId])))
    default:
