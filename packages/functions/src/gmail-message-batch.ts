...
async function modifyMessage(
  ...
  if (!res.ok) throw new Error(`modify failed for ${messageId}: ${res.status}`)
}
...
export const handler: Handler = async (event) => {
  ...
  let req: MessageOperationRequest
  try {
  ...
  if (!req.messageIds?.length || !req.action) {
    return { statusCode: 400, body: JSON.stringify({ error: 'messageIds and action required' }) }
  }
